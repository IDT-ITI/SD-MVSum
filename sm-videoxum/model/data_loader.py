...
class VideoData(Dataset):
    def __init__(self, mode, dataset='SM_VideoXum'):
        ...
        """
        self.mode = mode
        if dataset == 'SM_VideoXum':
            self.filename = './dataset/sm_videoxum.h5'
            self.dataset_split = './dataset/sm_videoxum_split.json'
        else:
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(self.filename, 'r') as hdf:
            ...
            text_features = torch.from_numpy(hdf[video_name + '/qwen2_embeddings'][()]).float()
...
def get_loader(mode, dataset='SM_VideoXum'):
    """ Loads the dataset.
