...
class Solver(object):
        ...
        for epoch_i in range(self.config.epochs):
            ...
            for _ in trange(num_batches, desc='Batch', ncols=80, leave=False):
                ...
                for _ in trange(self.config.batch_size, desc='Video', ncols=80, leave=False):
                    video_embeddings, text_embeddings_full, transcript_embeddings, gtscores = next(iterator)
    ...
    def evaluate(self, dataloader=None):
        """"
        Evaluates the model on a given DataLoader using F-score.
        ...
        for video_embeddings, text_embeddings_full, transcript_embeddings, gtscores in dataloader:
            video_embeddings = video_embeddings.to(self.config.device)
            text_embeddings_full = text_embeddings_full.to(self.config.device)
            transcript_embeddings = transcript_embeddings.to(self.config.device)
            ...
            for i in range(self.config.annotations):
                ...
                f_score_video.append(f_score)

            if f_score_video:
                fscore_history.append(np.mean(f_score_video))
