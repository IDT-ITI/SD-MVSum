...
from model.utils.evaluation_metrics import evaluate_summary as evaluate
...
class Solver(object):
    def __init__(self, config=None, train_loader=None, val_loader=None, test_loader=None):
        """
        Class builds, trains, and evaluates the SD-MVSum model.
    ...
    def build(self):
        """
        Builds and initializes the SD-MVSum summarization model and optimizer.
        Loads pretrained weights if available; otherwise applies weight initialization.
        return None
        """

        self.model = SD_MVSum(input_size=self.config.input_size,
                                          ...
                                          transcript_weights=self.config.transcript_weights).to(self.config.device)
    ...
    def train(self):
        ...
        for epoch_i in range(self.config.epochs):
            ...
            for _ in trange(num_batches, desc='Batch', ncols=80, leave=False):
                ...
                for _ in trange(self.config.batch_size, desc='Video', ncols=80, leave=False):
                    video_embeddings, text_embeddings, transcript_embeddings, gtscores, _, _ = next(iterator)
                    ...
                    transcript_embeddings = transcript_embeddings.to(self.config.device)
                    transcript_embeddings = transcript_embeddings.squeeze()
                    gtscores = gtscores.to(self.config.device)
                    gtscores = gtscores.squeeze(0)

                    score = self.model(video_embeddings, text_embeddings, transcript_embeddings)

                    loss = self.criterion(score.squeeze(0), gtscores)
                    loss.backward()
        ...
        f.write('   Best Val sRho {0:0.5} @ epoch{1}\n'.format(best_sRho, best_f1score_epoch))
        f.flush()
        f.close()
    ...
    def evaluate(self, dataloader=None):
        """"
        Evaluates the model on a given DataLoader using F-score and rank correlation coefficients.
        ...
        for video_embeddings, text_embeddings, transcript_embeddings, gtscores, gt_summary, cps in dataloader:
            video_embeddings = video_embeddings.to(self.config.device)
            text_embeddings = text_embeddings.to(self.config.device)
            transcript_embeddings = transcript_embeddings.to(self.config.device)

            nfps = np.array([cp[1] - cp[0] for cp in cps]).astype(int)
            nfps = nfps.tolist()
            ...
            n_frames = video_embeddings.shape[0]
            picks = np.array([i for i in range(n_frames)])
            machine_summary = generate_summary(score, cps, n_frames, nfps, picks)
            f_score, kTau, sRho = evaluate(machine_summary, gt_summary, score, gtscores)
    ...
    def test(self, ckpt_path):
        ...
        print('   TEST S-MrHiSum F-score {0:0.5}'.format(test_fscore[0]))
        print('   TEST S-MrHiSum kTau {0:0.5}'.format(test_fscore[1]))
        print('   TEST S-MrHiSum sRho {0:0.5}'.format(test_fscore[2]))
        print("------------------------------------------------------")


        f = open(os.path.join(self.config.save_dir_root, 'results.txt'), 'a')
        f.write("Testing on Model " + ckpt_path + '\n')
        f.write('Test F-score ' + str(test_fscore[0]) + '\n')
        f.write('Test kTau ' + str(test_fscore[1]) + '\n')
        f.write('Test sRho ' + str(test_fscore[2]) + '\n')
